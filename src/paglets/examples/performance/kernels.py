...
def _memory_copy_kernel(duration_seconds: float) -> BenchmarkMetric:
    ...
    elapsed = max(time.perf_counter() - started, 1e-9)
    return BenchmarkMetric(
        "memory-copy", elapsed, operations, operations / elapsed, bytes_processed, bytes_processed / elapsed
    )
...
def _memory_scan_kernel(duration_seconds: float) -> BenchmarkMetric:
    ...
    elapsed = max(time.perf_counter() - started, 1e-9)
    return BenchmarkMetric(
        "memory-scan", elapsed, operations, operations / elapsed, bytes_processed, bytes_processed / elapsed
    )
