...
def test_lazy_resident_service_idle_deactivates_but_stays_discoverable(tmp_path):
    ...
    try:
        ...
        assert host.get_proxy("service.server-info") is None
        assert (
            context.require_contract(SERVER_INFO, operation=GET_SUMMARY, scope=ServiceScope.MESH).record.proxy.agent_id
            == "service.server-info"
        )
    finally:
        host.stop()
...
def test_eager_resident_service_starts_immediately(tmp_path):
    ...
    try:
        ...
        assert (
            PagletContext(host)
            .require_contract(SERVER_INFO, operation=GET_SUMMARY, scope=ServiceScope.MESH)
            .call(GET_SUMMARY)
        )
    finally:
        host.stop()
...
def test_default_launch_config_starts_eager_mesh_info(tmp_path):
    ...
    try:
        ...
        snapshot = (
            PagletContext(host)
            .require_contract(MESH_INFO, operation=GET_SNAPSHOT, scope=ServiceScope.MESH)
            .call(GET_SNAPSHOT)
        )
...
def test_concurrent_first_calls_create_one_lazy_resident_service(tmp_path):
    ...
    try:
        ...
        def call_summary():
            return (
                context.require_contract(SERVER_INFO, operation=GET_SUMMARY, scope=ServiceScope.MESH)
                .call(GET_SUMMARY)
                .service_agent_id
            )
