...
def test_git_update_blocks_restart_when_uv_sync_fails(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(git_update, "_run_git", fake_run_git)
    monkeypatch.setattr(
        git_update, "_run_uv_sync", lambda _repo_root: git_update.GitCommandResult(1, stderr="sync failed")
    )
    ...
    threads = [
        threading.Thread(
            target=lambda: results.append(
                git_update.update_checkout(repo, process_start_head=old_head, lock_timeout=1.0)
            )
        )
        for _ in range(2)
    ]
